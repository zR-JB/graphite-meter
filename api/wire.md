...
| S→C | `BYTES_RECEIVED,<n>` | ws/wt (upload) | Optional server-measured running byte count (`n` uint64), for server-side upload measurement. |
| S→C | `UPLOAD_COMPLETE,<n>` | ws/wt (upload) | Final server-measured byte total. |
