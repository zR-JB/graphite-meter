...
| S→C | `BYTES_RECEIVED,<n>;TIME,<nanos>` | ws/wt (upload) | Server-measured running byte count. `<n>` (uint64) is the cumulative drained total; `<nanos>` (uint64) is the server's **active measurement time** — ns the server was *actually draining bytes* for this id, with dead zones (connection establishment, a lane reconnect, an idle stall) **excluded** — sampled at the same instant as `<n>`. The client derives upload rate as `Δn / Δnanos` over this server active clock — not its own frame-arrival clock and **not a wall span across frames** — so the live curve matches the headline at any push cadence and a stall/reconnect/early-finish can't stretch the denominator. (NB: unlike `PONG`'s `<nanos>`, which is a raw clock, this is accumulated active-transfer time.) |
| S→C | `UPLOAD_COMPLETE,<n>;TIME,<nanos>` | ws/wt (upload) | Final server-measured byte total, with the final active measurement time (`<nanos>`). Same `;TIME` semantics as `BYTES_RECEIVED`. |
