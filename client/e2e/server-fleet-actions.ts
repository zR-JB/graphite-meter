...
import type { FleetServer } from "./server-fleet";

export async function configureFleet(
  fleet: FleetServer[],
  page: Page,
  ids: string[],
  duration = 1500,
  ...
  await page.addInitScript(
    ({ ids, servers, duration, config, latencySelection, seedOnce }) => {
      if (seedOnce && localStorage.getItem("graphite-meter:v1")) return;
