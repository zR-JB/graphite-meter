...
test("wire presentation labels the estimate and exposes the complete model", () => {
  const view = presentWireEstimate(estimate, "lan", (rate) => `${rate} B/s`);
  ...
  expect(view.tooltip).toContain("Profile: lan; transport: HTTP/3 QUIC");
...
test("loopback is explicitly unavailable rather than an identity estimate", () => {
  const view = presentWireEstimate(
    { ...estimate, available: false, totalMultiplier: 1 },
    "loopback",
    String,
  );
