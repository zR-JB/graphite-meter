...
import type { InfraInfo, PhaseActivity, RunnerConfig, StallInfo, TransportDiscovery, TransportKind } from "./contract";
import type { FetchThroughputTarget } from "../api/endpoints";
import { DEFAULT_CONFIG } from "../state/defaults";
type ThroughputAdvertisement = Parameters<typeof classifyTransportDiscovery>[0][number];
type LatencyAdvertisement = Parameters<typeof classifyTransportDiscovery>[1][number];
...
const transfer = (id: string, origin: string, protocol: FetchThroughputTarget["protocol"], tls: boolean): FetchThroughputTarget => ({
  id,
  origin,
...
const discovery = (throughput: ThroughputAdvertisement[], latency: LatencyAdvertisement[] = [], pageOrigin = "http://meter:7246", pageSecure = false, pageProtocol = "http/1.1") =>
  classifyTransportDiscovery(throughput, latency, pageOrigin, pageSecure, pageProtocol);

const phaseActivity = (stage: PhaseActivity["stage"], transfer: PhaseActivity["transfer"] = [], loadedLatency = false): PhaseActivity => ({ stage, transfer, loadedLatency });
test("proxy endpoints resolve relative to preflight and negotiate the browser hop", () => {
  const catalog = discovery([{ baseUrl: ".", transport: "fetch-stream", protocol: "negotiated" }], [{ baseUrl: ".", transport: "websocket" }], "https://meter.example", true, "h2");
  ...
  expect(selectLatencyTarget(catalog, "auto")?.origin).toBe("https://meter.example");
});
test("deterministic native target wins when self resolves to the same origin", () => {
  ...
  expect(targetOfKind(catalog.throughput["https://meter.example"], "fetch-stream")?.protocol).toBe("http1");
...
test("native endpoints remain deterministic and mixed content stays blocked", () => {
  ...
  expect(selectThroughputTarget(catalog, "https://meter:7248")?.protocol).toBe("http2");
...
test("an IPv6 origin resolves each of its mechanisms", () => {
  ...
  expect(selectThroughputTarget(catalog, origin)?.transport).toBe("fetch-stream");
  expect(selectThroughputTarget(catalog, `${origin}::wt`)?.transport).toBe("webtransport");
  expect(selectLatencyTarget(catalog, `${origin}::wt`, true)?.transport).toBe("webtransport");
  expect(selectLatencyTarget(catalog, origin, true)?.transport).toBe("webtransport");
});
test("WebTransport folds onto its origin and leads latency auto-selection", () => {
  ...
  expect(wtStreams && "wtDownload" in wtStreams.routes ? wtStreams.routes.wtDownload : undefined).toBe(ROUTES.wtDownload);
  expect(datagram?.id).toBe("https://meter:7249::wtdg");
  expect(datagram?.transport).toBe("webtransport-datagram");

  expect(selectThroughputTarget(catalog, "auto")?.transport).toBe("fetch-stream");
  expect(selectThroughputTarget(catalog, "https://meter:7249::wt")?.transport).toBe("webtransport");
  expect(selectThroughputTarget(catalog, "https://meter:7249::wtdg")?.transport).toBe("webtransport-datagram");
  expect(selectThroughputTarget(catalog, "https://meter:7249")?.transport).toBe("fetch-stream");
...
test("one origin advertising both latency buses keeps the WebSocket fallback", () => {
  ...
  expect(selectLatencyTarget(catalog, "auto", true)?.transport).toBe("webtransport");
  expect(selectLatencyTarget(catalog, "auto", false)?.transport).toBe("websocket");
  expect(selectLatencyTarget(catalog, "https://meter::wt", true)?.transport).toBe("webtransport");
  expect(selectLatencyTarget(catalog, "https://meter", false)?.transport).toBe("websocket");
});
...
test("a legacy latency target without a transport remains a WebSocket bus", () => {
  const catalog = discovery([{ baseUrl: ".", transport: "fetch-stream", protocol: "negotiated" }], [{ baseUrl: "." }], "https://meter.test", true);
...
test("browser protocol verification is independent of server probe evidence", () => {
  ...
  const negotiated = transfer("https://meter", "https://meter", "negotiated", true);
...
test("idle target ownership includes protocol and public origin", () => {
  ...
  expect(throughputTargetKey({ ...target, origin: "https://other-meter" })).not.toBe(throughputTargetKey(target));
});
test("clear loopback targets stay usable from HTTPS", () => {
  for (const host of ["localhost", "meter.localhost", "127.42.0.9", "[::1]"]) {
    const target = transfer("http1-clear", `http://${host}:7246`, "http1", false);
    expect(discovery([target], [], "https://ui.example", true).throughput[target.origin].state).toBe("advertised");
  }
...
test("real backend: probe refresh keeps the negotiated protocol per role, and the upload stage opens its progress channel before any POST lane", async () => {
  ...
  class FakeWorker {
    ...
    constructor(url: URL) {
      ...
      this.kind = path.includes("upload-progress") ? "progress" : path.includes("upload-worker") ? "upload" : path.includes("download-worker") ? "download" : "ping";
      if (this.kind === "ping") pingWorker = this;
      if (this.kind === "upload" || this.kind === "download") transferWorkers.push(this);
    }

    postMessage(message: { type: string; url?: string } & Record<string, unknown>): void {
      if (this.kind === "ping") pingMessages.push(message);
      if (message.type !== "start" && message.type !== "measure") return;
      if (message.type === "start" && message.url) workerStarts.push({ kind: this.kind, url: message.url });
      if (this.kind === "ping") {
        queueMicrotask(() => {
  ...
  const probeWithRtts = async (probe: Promise<InfraInfo>, rtt: number): Promise<InfraInfo> => {
    let settled = false;
  ...
  const restoreProbe = stubProbeEnvironment(
    (async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/upload/session")) return Response.json({ uploadId: "gmu_test" });
  ...
  try {
    const { RealBackend, TransportUnavailableError } = await import("./RealRunner");
    ...
    const host = {
      ...
      elapsed: 0,
      emit(event) {
        if (event.type === "transportDiscovery") discoveries.push(event.discovery);
      },
      fail() {},
    ...
    await expect(backend.probe(config)).rejects.toBeInstanceOf(TransportUnavailableError);
    expect(discoveries.at(-1)?.throughput["https://meter.test:7248"].state).toBe("advertised");
    config.transports.throughputTarget = "http://meter.test:7246";
    ...
    expect(preflightUrls.every((url) => url.startsWith("/preflight?client=web&client_version="))).toBe(true);
    expect(workerStarts.filter(({ kind }) => kind === "ping").every(({ url }) => url === "ws://meter.test:7246/ws/ping")).toBe(true);
    expect(workerStarts.some(({ url }) => url.includes("8765"))).toBe(false);
    expect(discoveries[0].throughput["https://meter.test:7248"]).toBeUndefined();
    expect(discoveries[2].throughput["https://meter.test:7248"].state).toBe("advertised");
    ...
    const latencyProbe = await probeWithRtts(backend.probe(config, undefined, "latency"), 7);
    expect(latencyProbe.preTestPingMs).toBe(7);
    expect(fetchUrls.slice(fetchStart)).toHaveLength(2);

    config.transports.throughputTarget = "https://proxy.test";
    browserProtocol = "";
    const proxyWithoutTiming = await backend.probe(config, undefined, "throughput");
    expect(proxyWithoutTiming.selectedThroughputProtocol).toBe("negotiated");
    expect(targetOfKind(discoveries.at(-1)!.throughput["https://proxy.test"], "fetch-stream")?.protocol).toBe("negotiated");

    browserProtocol = "h2";
    const proxyThroughput = await backend.probe(config, undefined, "throughput");
    expect(proxyThroughput.selectedThroughputProtocol).toBe("http2");
    const proxyLatency = await probeWithRtts(backend.probe(config, undefined, "latency"), 9);
    ...
    expect(workerStarts.at(-1)?.url).toBe("http://meter.test:7246/upload/progress?id=gmu_test");

    progressWorker!.emit({ type: "open" });
    await preparation;
    expect(started).toEqual(["progress", "upload", "upload", "upload", "upload", "upload", "upload"]);
    ...
    if (!staleEnding) throw new Error("stale upload finalization did not return a promise");
    backend.onAbort();
...
function stubProbeEnvironment(fetchImpl: typeof fetch, options: { location?: string; protocol?: string | (() => string) } = {}): () => void {
  const buildGlobals = globalThis as typeof globalThis & Record<string, unknown>;
  ...
  performance.getEntriesByName = () =>
    [
      {
        nextHopProtocol: typeof options.protocol === "function" ? options.protocol() : (options.protocol ?? "http/1.1"),
      },
  ...
  return () => {
    ...
    for (const key of Object.keys(BUILD_TOKENS)) Reflect.deleteProperty(buildGlobals, key);
  };
}
...
test("a WebTransport-less browser is refused by mechanism, not by availability", async () => {
  ...
  expect(selectThroughputTarget(catalog, "auto")?.transport).toBe("webtransport");
  expect(selectThroughputTarget(catalog, "auto", false)).toBeNull();

  const globals = globalThis as typeof globalThis & Record<string, unknown>;
  const realWebTransport = Object.getOwnPropertyDescriptor(globalThis, "WebTransport");
  ...
  try {
    ...
    restore();
    if (realWebTransport) Object.defineProperty(globalThis, "WebTransport", realWebTransport);
  }
});
test("a superseded probe does not publish its discovery", async () => {
  ...
  const restore = stubProbeEnvironment((async (input: RequestInfo | URL) => {
    ...
    if (url.includes("/preflight")) {
      preflights++;
      if (preflights === 1) await new Promise<void>((resolve) => (releaseFirst = resolve));
  ...
  try {
    ...
    for (let turn = 0; turn < 20 && preflights < 1; turn++) await Promise.resolve();
...
test("a throughput-role probe keeps the latency bus the last check committed to", async () => {
  ...
  class PingBusWorker {
    ...
    postMessage(message: { type: string; transport?: string }): void {
      ...
      if (this.transport === "websocket") queueMicrotask(() => this.emit({ type: "ready" }));
    }
  ...
  try {
    ...
    globalThis.Worker = realWorker;
    if (realWebTransport === undefined) Reflect.deleteProperty(globals, "WebTransport");
    else globals.WebTransport = realWebTransport;
    restore();
  }
}, 15000);
test("every transport has a registry row saying how it is driven", () => {
  const kinds: TransportKind[] = ["fetch-stream", "websocket", "webtransport", "webtransport-datagram"];
  ...
  expect(kindsForRole("throughput")).toEqual(["fetch-stream", "webtransport", "webtransport-datagram"]);
