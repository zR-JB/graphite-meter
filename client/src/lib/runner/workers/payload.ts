// The incompressible source block every upload path writes, so a compressing
// hop cannot flatter the measurement.

/** crypto.getRandomValues' hard per-call byte quota. */
const RNG_CHUNK_BYTES = 65536;
/** Large enough that the fill cost never lands on a write, small enough to
 *  hold on a low-memory device. */
export const FILL_BLOCK_BYTES = 4 * 1024 * 1024;

const filled = new Map<number, Uint8Array<ArrayBuffer>>();

/** The reusable block, filled once with CSPRNG bytes in quota-sized chunks.
 *  Memoised per size per worker realm: the fill never lands on a write path. */
