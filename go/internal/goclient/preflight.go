...
func getProbe(ctx context.Context, hc *http.Client, target *wire.ThroughputTarget) (wire.Probe, error) {
...
func getLatencyProbe(ctx context.Context, hc *http.Client, target *wire.LatencyTarget) (wire.Probe, error) {
	...
	if p.ProtocolNegotiated != "http/1.1" {
		return wire.Probe{}, fmt.Errorf("latency target negotiated %q, want http/1.1", p.ProtocolNegotiated)
	}
	return p, nil
}
