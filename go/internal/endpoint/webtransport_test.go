...
// the native client validates its ping cadence against the same constant.
func TestIdleBoundIsThePublishedContractValue(t *testing.T) {
	if got := time.Duration(wtIdleBound.Load()); got != wire.WTIdleBound {
		t.Errorf("idle bound = %v, want the published %v", got, wire.WTIdleBound)
	}
}
