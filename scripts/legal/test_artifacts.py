...
class SourceArchiveTests(unittest.TestCase):
    ...
    def test_deterministic_split_source_offer_excludes_unlisted_project_and_local_files(self) -> None:
        ...
        provenance = [Provenance(name="sample", localPaths=["manual.txt"],
        ...
        names = ("third_party/manual/sample/manual.txt", "LEGAL_INVENTORY.json", "PROVENANCE.json",
                 "README.txt")
        self.assertEqual(set(files), {prefix + name for name in names})
    ...
    def test_archive_uses_resolved_browser_and_go_replacement_paths(self) -> None:
        ...
        self.scopes["server/browser"] = [Component("svelte", "5", "npm", source_path=npm),
                                         Component("../replacement", ecosystem="go", source_path=go)]
        ...
        inventory = files["graphite-meter_development_third-party-source/LEGAL_INVENTORY.json"]
        self.assertNotIn(str(self.root).encode(), inventory)
    ...
    def test_manual_archive_legal_files_and_relative_sources_are_required(self) -> None:
        ...
        entry = Provenance(name="font", localLegalFiles=[LegalFile("manual/LICENSE")],
                           correspondingSource="third_party/manual/font")
        output = self.root / "sources.tar.gz"
        third_party_source_bundle(self.root, Project(), "development", self.scopes, [entry], output)
        font = "graphite-meter_development_third-party-source/third_party/manual/font/LICENSE"
        self.assertIn(font, self.contents(output))
