import { StateManager, AppState, createStateMessage, formatBytes, formatBitrate, formatBits } from './utils.js';

class WebTransportDownloadClient {
    constructor() {
    ...
    logStateTransition(stateChange) {
        ...
        switch (to) {
            ...
            case AppState.WARMUP:
                this.logMessage(`Preparing download...`, 'info');
                break;
            case AppState.RUNNING:
                this.logMessage(`Download started with ${this.config.workers} worker(s)`, 'success');
                break;
            case AppState.STOPPING:
                this.logMessage(`Stopping download...`, 'warning');
                break;
            case AppState.STOPPED_WITH_RESULTS:
                this.logMessage('Download stopped', 'success');
                break;
    ...
    initializeUI() {
        ...
        document.getElementById('stop-btn').onclick = () => this.stopDownload();
    ...
    updateUrlForTransportMode() {
        ...
        if (transportMode === 'webtransport') {
            defaultUrl = `${currentUrl.protocol}//${currentUrl.host}/api/download`;
        } else {
            defaultUrl = `${currentUrl.protocol}//${currentUrl.host}/download`;
        }
    ...
    async initializeWorkers() {
        ...
        this.downloadWorkers.forEach(w => w.terminate());
        this.downloadWorkers = [];
        if (this.uiWorker) {
        ...
        for (let i = 0; i < this.config.workers; i++) {
            const workerScript = this.config.transportMode === 'webtransport' 
                ...
                : './workers/fetch-download-worker.js';
                
            const worker = new Worker(workerScript, { type: 'module' });
            worker.onmessage = (e) => this.handleDownloadMessage(e);
            worker.onerror = (err) => {
                console.error(`[MAIN] Download worker ${i+1} error:`, err);
    ...
    handleDownloadMessage(e) {
        ...
        if (type === 'progress') {
            ...
            // If all workers are done, transition to completed state after a short grace period
            // to allow final stats to be processed, then auto-disconnect
            if (this.activeWorkers <= 0) {
                setTimeout(async () => {
                    if (this.stateManager.isState(AppState.RUNNING)) {
                        await this.stateManager.transition(AppState.STOPPED_WITH_RESULTS, 'Download completed');
                        // Auto-disconnect after test completion
                        setTimeout(() => this.disconnect(), 1000);
                    } else if (this.stateManager.isState(AppState.STOPPING)) {
                        await this.stateManager.transition(AppState.STOPPED_WITH_RESULTS, 'Download stopped');
                        // Auto-disconnect after test completion
                        setTimeout(() => this.disconnect(), 1000);
                    }
                }, 200); // 200ms grace period for final stats
            }
        }
    }
    ...
    broadcastStateToWorkers(stateChange) {
        ...
        this.downloadWorkers.forEach(w => w.postMessage(message));
        if (this.uiWorker) {
    ...
    updateUIForState(state) {
        ...
        switch (state) {
            ...
            case AppState.WARMUP:
                ...
                break;
            case AppState.RUNNING:
                updateStatus('Downloading...', 'running');
                startTestBtn.disabled = true;
                disconnectBtn.disabled = false;
                stopBtn.disabled = false;
                urlInput.disabled = true;
    ...
    async startTest() {
        ...
        this.config.sizeBytes = Math.max(1, Math.floor(parseFloat(document.getElementById('size').value) * 1000 * 1000)); // Base 1000, minimum 1 byte
        this.config.workers = parseInt(document.getElementById('workers').value, 10);
        ...
        await this.stateManager.transition(AppState.CONNECTING, 'Starting test', { url: this.config.url });
        await this.stateManager.transition(AppState.CONNECTED, 'Connected', { url: this.config.url });
    ...
    async connect() {
        ...
        await this.stateManager.transition(AppState.CONNECTING, 'User initiated connection', { url: this.config.url });
        await this.stateManager.transition(AppState.CONNECTED, 'Connection established', { url: this.config.url });
    }

    async startDownload() {
        if (!this.stateManager.canStartDownload()) {
            this.logMessage('Cannot start download in current state', 'error');
            return;
        }

        // Re-read the size from the input field to allow changing it after connecting
        const totalSize = Math.max(1, Math.floor(parseFloat(document.getElementById('size').value) * 1000 * 1000)); // Base 1000, minimum 1 byte
        ...
        await this.stateManager.transition(AppState.WARMUP, 'User started download');
        await this.stateManager.transition(AppState.RUNNING, 'Download running');

        this.downloadWorkers.forEach((w, i) => {
            const workerSize = sizePerWorker + (i === this.config.workers - 1 ? remainder : 0);
            w.postMessage({
                type: 'start',
                data: {
                    url: this.config.url,
    ...
    async stopDownload() {
        if (!this.stateManager.canStopDownload()) {
            this.logMessage('Cannot stop download in current state', 'error');
            return;
        }

        await this.stateManager.transition(AppState.STOPPING, 'User requested stop');
        this.downloadWorkers.forEach(w => w.postMessage({ type: 'stop' }));
        
        // If workers are already done (which might happen very fast), transition to results state
        if (this.activeWorkers <= 0) {
            await this.stateManager.transition(AppState.STOPPED_WITH_RESULTS, 'Download stopped');
        }
    }
    ...
    updateStatsDisplay(stats) {
        // Display total downloaded with dynamic units
        ...
        document.querySelector('#total-mb').parentNode.querySelector('.stat-unit').textContent = unitSI;

        const totalIEC = formatBytes(stats.totalBytes, { base: 1024, decimals: 2, iec: true });
        const [valIEC, unitIEC] = totalIEC.split(' ');
        document.getElementById('total-mib').textContent = valIEC;
        document.querySelector('#total-mib').parentNode.querySelector('.stat-unit').textContent = unitIEC;
        
        // Display network speeds with auto-scaling
        ...
        // Bytes per second (SI - 1000-based)
        ...
        document.getElementById('speed-bytes-si-unit').textContent = unitSI_speed + '/s';

        // Bytes per second (IEC - 1024-based)
        const bytesIEC = formatBytes(stats.speeds.Bps, { base: 1024, decimals: 2, iec: true });
        const [valIEC_speed, unitIEC_speed] = bytesIEC.split(' ');
        document.getElementById('speed-bytes-iec').textContent = valIEC_speed;
        document.getElementById('speed-bytes-iec-unit').textContent = unitIEC_speed + '/s';
        
        // Display active workers count
        document.getElementById('active-workers').textContent = this.activeWorkers;
    ...
    updateStatusDisplay(state) {
        const statusMap = {
            ...
            [AppState.RUNNING]: { text: 'Downloading', class: 'running' },
            [AppState.STOPPING]: { text: 'Stopping...', class: 'stopping' },
            [AppState.STOPPED_WITH_RESULTS]: { text: 'Download Complete', class: 'completed' },
...
window.app = new WebTransportDownloadClient();