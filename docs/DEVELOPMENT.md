...
| `just client-e2e`        | `bun run test:e2e` — rebuilds the bundle, then runs the Playwright browser tests in Chromium and Firefox. Serves the bundle alone, so no test here reaches a backend. Slow (~45s), so it is outside `just ci`.               |
| `just client-e2e-live`   | Boots the measurement server and moves bytes over the real transports from Chromium, through the production lanes. The only coverage the browser half of WebTransport has. Needs Go and openssl; the certificate is generated per run. ~5s.               |
...
| `just ci-full`           | `ci` plus `client-e2e` and `client-e2e-live`. The Docker smoke job and the cross-build matrix stay CI-only (they need a container runtime or other toolchains).                        |
