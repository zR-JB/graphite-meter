...
| `--ping`                   | `medium`                | Ping cadence: `instant` (80ms) / `medium` (250ms) / `slow` (600ms), or a raw Go duration. Over the WebTransport latency bus the cadence is capped at `15s` — half the server's 30-second WebTransport idle bound, past which the bus is reaped between pings — and `auto` is held to the cap too, since it may resolve to WebTransport. The WebSocket bus has no idle timer and takes any positive cadence. |
