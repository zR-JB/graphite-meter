...
| `GM_MAX_SESSIONS_PER_CLIENT`            | `--max-sessions-per-client`            | `4`              | Per-client WebTransport sessions. A session holds a measurement slot for a whole test, so it carries its own budget; raise it where many users share one address (CGNAT). Must not exceed the per-client measurement limit. |
